# to do actual crawling we can use the method urlopen from urllib2
# and the read method to actually extract the source code of that webpage
# For that import the library urllib2 and then use the urlopen method.
# It is illustrated in my other python file named crawler.py.
# Due to politeness issues i have used code for the procedure get_page()
# which is present on the website udacity.com
...
def crawling_web(seed,max_depth):
    tocrawl = [seed]
    crawled = []
    next_depth=[]
    i=0
    while (i<=max_depth and tocrawl):
        page = tocrawl.pop()
        if page not in crawled:
            union(next_depth, get_links(get_page(page)))
            crawled.append(page)
        if not tocrawl:
...
print crawling_web("http://top.contributors/forbiddenvoid.html",2)
